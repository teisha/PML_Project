...
The following code goes back to the original training data and pulls all the records, but only the `r length(colnames)` columns that were identified as "important".  Since this is just a verification that we're on the right track before running the main model, rows are also pruned before generating the model, in order to reduce processing time.
...
The training results run against the validation data set look good enough to build the bigger model and run the test.  

## Final Results

The code applied to the test set appears below, but since we weren't given the results in the test data set, there is no comparison to the actual results.  
